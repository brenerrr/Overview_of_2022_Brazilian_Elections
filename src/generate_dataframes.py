...
# Simplify states geometry
gdf_states = gpd.read_file(os.path.join('files', 'BR_UF_2021.shp'))
gdf_states["geometry"] = (
    gdf_states.to_crs(gdf_states.estimate_utm_crs()).simplify(2000).to_crs(gdf_states.crs)
)
gdf_states = gdf_states.rename({'SIGLA': 'SG_UF'}, axis=1)
gdf_states = gdf_states.to_crs(4326)
with open(os.path.join('files', 'states_geo.json'), 'w') as file:
    file.write(gdf_states.to_json(indent=2))

# Simplify counties geometry
gdf_counties = gpd.read_file(os.path.join('files', 'BR_Municipios_2021.shp'))
...
gdf_counties = gdf_counties.to_crs(4326)
with open(os.path.join('files', 'counties_geo.json'), 'w') as file:
